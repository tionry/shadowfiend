(function () {
    window.RTCMultiConnection = function (channel, extras) {
            ...
            if (!self.openSignalingChannel) {
                ...
                self.openSignalingChannel = function (config) {
                    ...
                    if (channel != self.channel || (self.isInitiator && channel == self.channel)){
                        ...
                        socket.isCreater = true;
                    }
                    else{
                        socket.isCreater = false;
                    }
        ...
        function init() {
            ...
            self.config = {
                ...
                onleave: function (userid, extra, ejected) {
                    self.onleave(userid, extra, ejected);
                },
        ...
        function captureUserMedia(callback) {
            ...
            var mediaConfig = {
                ...
                onerror: function () {
					...
					var length = window.mySockets.length;
					for(var i = 0;i < length;++i){
						if(window.mySockets[i]){
							if(window.mySockets[i].isCreater){
								window.mySockets[i].remove();
								window.mySockets[i].unauth();
							}
							window.mySockets[i].onmessage = function(){
								return;
							};
							delete window.mySockets[i];
						}
					}
        ...
        this.onleave = function (userid, extra, ejected) {
            self.onUserLeft(userid, extra, ejected);
        };
    ...
    function RTCMultiSession(config) {
        ...
        function openDefaultSocket() {
			...
			window.mySockets.push(defaultSocket);
        }
        ...
        function openSubSocket(_config) {
            ...
            function initPeer(offerSDP) {
				...
				if(config.onRemoteStream){
					peer.myOnRemoteStream = config.onRemoteStream;
				}
				window.peerArray[_config.extra.username] = peer;
				window.peerUserArray.push(_config.extra.username);
            }
            ...
            function afterRemoteStreamStartedFlowing() {
				if(peer.myOnRemoteStream){
					peer.myOnRemoteStream({
                        mediaElement: mediaElement,
            ...
            function onSessionOpened() {
                ...
                if (direction === Direction.ManyToMany && isbroadcaster && channels.split('--').length > 3) {
                    if (!_config.alreadySentDetails) {
						...
						var myContext = {};
						if(socket){
							myContext['newParticipant'] = socket.channel;
						}
						myContext['userid'] = self.id;
						myContext['extra'] = extra;
                        defaultSocket.send(myContext);
                    }
                }
            }
            ...
            function socketResponse(response) {
                ...
                if (response.left) {
                    ...
                    if (response.closeEntireSession) {
						...
						ejected = true;
						config.connection.myLocalStream.stop();
                        leaveARoom();
                    } else if (socket) {
                        socket.send({
                            left: true,
                    ...
                    if (config.onleave) config.onleave(response.id, response.extra, ejected);
                }
        ...
        function leaveARoom(channel) {
            ...
            if (!channel) {
				...
				function weisuoFun(){
					length = window.mySockets.length;
					for(var i = 0;i < length;++i){
						if(window.mySockets[i]){
							if(window.mySockets[i].isCreater){
								window.mySockets[i].remove();

...
(function () {
    ...
    var upload = function (event) {
		...
		function () {
		    ...
		    var pos2 = filepath.lastIndexOf('\\'); (pos < pos2) && (pos = pos2);
    ...
    var newFile = function (event) {
		...
		type = $(event.target).attr('new-type');
        if (that.mode != app.FilesView.Mode.BelongSelf) {
            return;
        }
        if (type == 'up') {

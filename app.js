...
//		}
		...
		interviewDAO.getInterviewByName(data.interviewName, function(err, interview) {
			...
			for (i = 0; i < interview.problemlist.length; i++) {
				if (interview.problemlist[i].status == 'pushing') {
					problemName = interview.problemlist[i].name;
					break;
				}
			}
			if (!problemName) {
			...
			var path = '/' + socket.session.user.name + '/' + problemName + '@' + data.interviewName;
			docDAO.getDocByPath(socket.session.user._id, path, function(err, doc) {
				if (err) {
					return socket.emit('try-enter-interview', {err: err});
				}
				problemDAO.getProblemByName(problemName, function(err, problem) {
	...
	socket.on('send-end-round-info', function(data){
		if (!check(data, 'intervieweeList')) {
			return;
		}
		if (!socket.session) {
	...
	socket.on('send-end-interview-info', function(data){
		if (!check(data, 'intervieweeList')) {
			return;
		}
		if (!socket.session) {
	...
	socket.on('get-interviewee-problem-list', function(data) {
		...
		interviewDAO.getintervieweeproblem(data.interviewName, data.intervieweeName, function(err, problemNameList) {
			if (err) {
				return socket.emit('after-get-interviewee-problem-list', {err: err});
			}
			problemDAO.getProblemByNameList(problemNameList, function(err, problemList) {
				if (err) {
					return socket.emit('after-get-interviewee-problem-list', {err: err});
				}
